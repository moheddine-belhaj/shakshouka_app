...
import { ChevronRight, Shield, BookOpen, Tv, Mail, Search, X, Folder } from "lucide-react"
...
export default function ListPage() {
  const { files, categories, getCategoryLabel, getCategoryColor } = useStore()
  ...
  const getCategoryIcon = (category: FileCategory): React.ElementType => {
    return defaultCategoryIcons[category] || Folder
  }
  ...
  const filteredCategories = useMemo(() => {
    ...
    return categories.filter((category) => {
      ...
      const labelMatch = getCategoryLabel(category).toLowerCase().includes(query)
  ...
  }, [searchQuery, files, categories, getCategoryLabel])
              ...
              <Link key={category} href={`/list/${category}`}>
                <Card className="hover:border-primary/50 transition-colors cursor-pointer">
                  <CardContent className="flex items-center gap-4 py-4">
                    <div
                      className={`size-12 rounded-xl flex items-center justify-center ${getCategoryColor(category)}`}
                    >
            ...
            <div className="flex gap-1">
              {filteredCategories.map((category) => {
                ...
                return (
                  <div
                    key={category}
                    className={`h-8 rounded-full ${getCategoryColor(category)}`}
